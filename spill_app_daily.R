#--------------------------------------------------------------------------------
#--------------------------------------------------------------------------------
# This is the app that includes the daily discharge values.
...
ui <- dashboardPage(skin = "blue",
                    ...
                    dashboardSidebar(width = 250,
                                     ...
                                     dateInput("selected_date", 
                                               "Date of Spill (Choose a past date)"),
...
server <- function(input, output) {
  ...
  # gives real-time data for the date/time at the top right of the app
  ...
  # gives daily discharge value
  getDataFromDataFrame <- function(selected_station, selected_date) {
    ...
    result_data <- data.frame(
      ...
      "Daily Stream Discharge in cfs" = discharge$X_00060_00003, # instant discharge, column-specific
      "Long-term Mean Discharge in cfs" = gage_values$mean_flow[1], # 10-year average
      "Stream Velocity in mph" = gage_values$a_intercept[1] * discharge$X_00060_00003^gage_values$b_slope[1],
      "a (intercept)" = gage_values$a_intercept[1],
      "b (slope)" = gage_values$b_slope[1],
      "Contaminant Travel Time in Hours" = input$distance / (gage_values$a_intercept[1] * discharge$X_00060_00003^gage_values$b_slope[1]),
  ...
  observeEvent(input$run_something, {
    if (is.null(input$station) || is.null(input$selected_date)) {
